...
class RecurrentConvBlock(nn.Module):
    def __init__(self, in_ch, out_ch, t=4):
        super().__init__()
        self.t = t
        self.in_conv = nn.Conv2d(in_ch, out_ch, kernel_size=3, padding=1)
        self.shared_conv = nn.Conv2d(out_ch, out_ch, kernel_size=3, padding=1)
        self.gate_conv = nn.Conv2d(out_ch, out_ch, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        for _ in range(1, self.t):
            gate = self.sigmoid(self.gate_conv(h))
            h = self.relu(self.bn(self.shared_conv(h)*(1-gate) + x * gate))  # 门控残差迭代

        return h

class GRC_DSCNet(nn.Module):
    def __init__(
        self,
        n_channels,
        n_classes,
        ...
        self.conv1 = RecurrentConvBlock(3 * self.number, self.number)
        ...
        self.conv3 = RecurrentConvBlock(6 * self.number, 2 * self.number)
        ...
        self.conv5 = RecurrentConvBlock(12 * self.number, 4 * self.number)
        ...
        self.conv7 = RecurrentConvBlock(24 * self.number, 8 * self.number)
        ...
        self.conv13 = RecurrentConvBlock(12 * self.number, 4 * self.number)
        ...
        self.conv15 = RecurrentConvBlock(6 * self.number, 2 * self.number)
        ...
        self.conv17 = RecurrentConvBlock(3 * self.number, self.number)
